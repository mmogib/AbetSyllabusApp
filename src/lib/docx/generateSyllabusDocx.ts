...
function populateDesignationRow(row: Element, designation: string): void {
  ...
  setCellText(cells[2], normalizedDesignation === 'required' ? '√' : '');
  setCellText(cells[3], normalizedDesignation === 'selected-elective' ? '√' : '');
  setCellText(cells[4], normalizedDesignation === 'elective' ? '√' : '');
}
...
async function buildDocxBlob(draft: SyllabusDraft): Promise<Blob> {
  ...
  populateTemplateDocument(document, draft);
  zip.file(
    'word/document.xml',
  ...
  if (coreXml) {
    zip.file('docProps/core.xml', updateCoreTitle(coreXml, draft));
  }
